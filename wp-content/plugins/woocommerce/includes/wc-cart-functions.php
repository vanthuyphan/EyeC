...
function wc_add_to_cart_message( $products, $show_qty = false, $return = false ) {
	...
	$added_text = sprintf( _n( '%s has been added to your Store Try-On cart.', '%s have been added to your Store Try-On cart.', $count, 'woocommerce' ), wc_format_list_of_items( $titles ) );
	...
	if ( 'yes' === get_option( 'woocommerce_cart_redirect_after_add' ) ) {
		...
		$message   = sprintf( '<a href="%s" class="button wc-forward">%s</a> %s', esc_url( wc_get_page_permalink( 'cart' ) ), esc_html__( 'Set Up Appointment', 'woocommerce' ), esc_html( $added_text ) );
	}
